...
当前系统不支持文件上传，用户无法在对话中发送图片、文档等附件。需要提供文件上传接口，让用户上传文件后能在聊天中引用附件，实现多模态对话体验。

## What Changes

- **新增 `POST /upload` 接口** —— 接收 multipart 文件上传，按 `session_id` 隔离存储到 workspace 的 data 目录，返回 `DataBlock`（URLSource）供前端在后续对话中引用
...
#### Scenario: 上传成功
- **WHEN** 前端通过 `multipart/form-data` 上传文件，携带 `session_id` 和 `file` 字段
...
#### Scenario: 未携带 session_id
- **WHEN** 上传请求未提供 `session_id`
- **THEN** 返回 `{"code": 400, "msg": "缺少 session_id", "data": {}}`
