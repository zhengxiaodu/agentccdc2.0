...
def get_card_config(raw_input: Any) -> str:
    ...
    不传 cardType 时返回所有可用卡片类型列表（含 cardType、cardName、schemaFields），
    让大模型自行判断选择哪种图表；
    ...
    参数 (input dict):
        cardType (str, optional): 卡片类型。不传时返回所有可用卡片列表；
                                  传入具体值时返回该卡片的完整配置。
                                  可选值：bar（柱状图）、line（折线图）、pie（饼图）、
                                         table（技术指标表格）、metric（指标卡）
    ...
    try:
        ...
        card_type = inp.get("cardType") or inp.get("card_type")
        app_id = inp.get("appId") or inp.get("app_id") or "default"

        # 未传 cardType：返回所有可用卡片列表
        if not card_type:
            logger.info(f"[{GET_CARD_CONFIG}] 查询所有可用卡片列表")
            return _fetch_all_cards(app_id)

        # 传入 cardType：查询单个卡片配置
        logger.info(f"[{GET_CARD_CONFIG}] 查询卡片配置: cardType={card_type}")
        return _fetch_single_card(app_id, card_type)
...
def _fetch_all_cards(app_id: str) -> str:
    """获取所有可用卡片列表"""
    try:
...
def _fetch_single_card(app_id: str, card_type: str) -> str:
    """获取单个卡片配置"""
    try:
...
def get_custom_component_config(raw_input: Any) -> str:
    ...
    参数 (input dict):
        ...
        appId (str, optional): 应用ID，默认 "default"
